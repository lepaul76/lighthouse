...
for (const fnName_ of DELEGATED_FUNCTIONS) {
  ...
  describe(fnName, () => {
    it('should fail if called before connect', () => {
      expect(driver[fnName]).toThrow(/Driver not connected/);
    });
    ...
    it('should delegate to session', async () => {
      ...
      const actualResult = driver[fnName](...args);
...
describe('.evaluateAsync', () => {
  ...
  it('should delegate to runtime controller', async () => {
    await driver.connect();
    if (!driver._runtimeController) throw new Error('Runtime did not connect');

    const returnValue = 2;
    const evaluateAsyncMock = driver._runtimeController.evaluateAsync =
